...
async def handle_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Wallet ──
        ...
        await query.edit_message_text("💎 <b>Send your TRX wallet address:</b>", parse_mode="HTML")
        ...
        await query.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup([
                ...
                [InlineKeyboardButton("❌ Cancel",  callback_data="back_wallet")],
            ])
        )
    ...
    # ── Admin panel ──
        ...
        ctx.user_data["adm_state"] = "add_numbers"
        await query.edit_message_text(
            "📋 <b>Send numbers (one per line):</b>\n\n"
            "<code>number,country,service</code>\n\n"
            "Example:\n<code>2348012345678,Nigeria,WhatsApp\n2338012345678,Ghana,Facebook</code>",
            parse_mode="HTML"
        )
        ...
        await query.edit_message_text("🗑 <b>Send country name to delete its numbers:</b>", parse_mode="HTML")

    elif data == "adm_users":
        if not is_admin(user_id): return
        users = get_all_users()
        lines = "\n".join([f"• <code>{u['user_id']}</code> @{u['username'] or 'N/A'} — ${u['balance']:.5f}" for u in users[:30]])
        await query.edit_message_text(f"👥 <b>Total Users: {len(users)}</b>\n\n{lines}", parse_mode="HTML")
