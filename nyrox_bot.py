...
BOT_TOKEN        = "8706326366:AAH3ltbkml39u6jY-1u9uJMtYirqwXu5zko"
ADMIN_ID         = 6572004742
ADMIN_IDS        = (ADMIN_ID,)
...
# ── API Sources ───────────────────────────────────────────────
# Format A — uses fromdate/todate + number/cli fields
APIS_A = [
    {"name": "MO",    "url": "http://137.74.1.203/crapi/reseller/mdr.php",  "token": "QVdTR0NWfkJGUFRI"},           # ← Add token when ready
    {"name": "HADI",  "url": "http://147.135.212.197/crapi/had/viewstats",                                              "token": "R1JXQjRSQlVFbpFJQlZXfXZuckJmiZeIg5BUeXyKU4qCYVFDenFy"},           # ← Add URL + token when ready
]
...
def init_db():
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS country_payouts (
...
def get_country_payout(country):
    """Get payout for a specific country, fallback to default OTP reward"""
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT payout FROM country_payouts WHERE LOWER(country)=LOWER(?)", (country,))
    r = c.fetchone(); conn.close()
    return float(r["payout"]) if r else get_otp_reward()

def set_country_payout(country, payout):
    conn = get_conn()
    conn.execute("INSERT OR REPLACE INTO country_payouts (country, payout) VALUES (?,?)", (country, payout))
    conn.commit(); conn.close()

def get_all_country_payouts():
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT * FROM country_payouts ORDER BY country")
...
def build_group_card(country, flag, service, svc_icon, masked_num, otp):
    """New sleek card for group"""
    return (
        f"⚡ <b>NYROX</b>  ·  {flag} {country}  ·  {svc_icon} {service}\n"
        f"━━━━━━━━━━━━━━━━━━━━━\n"
...
def build_private_card(country, flag, service, svc_icon, full_num, otp, payout, balance):
    """New sleek card for private"""
    return (
        f"✅ <b>OTP RECEIVED</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━━\n"
        f"{flag} {country}  ·  {svc_icon} {service}\n"
        f"📞  <code>+{full_num}</code>\n"
        f"━━━━━━━━━━━━━━━━━━━━━\n"
        f"🔑  <b>{otp}</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━━\n"
        f"💰  +<b>${payout:.5f}</b>   💼  <b>${balance:.5f}</b>"
    )
...
def admin_keyboard():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🌍 Country Payouts",   callback_data="adm_country_payouts")],
    ])
...
async def check_membership(bot, user_id):
    try:
        ...
        if ch.status in ("left", "kicked", "banned"):
            return False
    ...
    try:
        ...
        if gr.status in ("left", "kicked", "banned"):
            return False
    except Exception as e:
        logger.warning(f"Group check error: {e}")
        return False
    return True
...
async def process_and_forward(bot, sms):
    ...
    try: api_payout = float(sms.get("payout") or 0)
    except Exception: api_payout = 0
    ...
    if number_row and number_row["status"] == "assigned":
        ...
        # Get payout: country-specific > API payout > default
        payout = get_country_payout(country)
        if api_payout > 0 and not get_all_country_payouts():
            payout = api_payout
        ...
        payout = api_payout if api_payout > 0 else get_otp_reward()
...
async def cmd_numbers(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ctx.user_data["selecting_country"] = True
    await update.message.reply_text(
        "🌍 <b>Type your Country:</b>\n\n"
        "<i>Example: </i><code>Nigeria</code>",
        parse_mode="HTML"
    )
...
async def handle_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Numbers flow ──
        ...
        ctx.user_data["selecting_service_for"] = country
        flag, _ = get_country_info(country)
        await query.edit_message_text(
            f"✅ Country: {flag} <b>{country}</b>\n\n"
            f"📡 <b>Type your Service:</b>\n\n"
            f"<i>Example: </i><code>WhatsApp</code>",
            parse_mode="HTML"
        )
    ...
    # ── Admin panel actions ──
        ...
        ctx.user_data["adm_state"] = "adm_typing_country"
        countries_list = [
        ...
        await query.edit_message_text(
            ...
            f"🌍 <b>Select or type Country:</b>\n"
            f"<i>Don't see yours? Just type it!</i>\n"
            f"<i>Example: </i><code>Tanzania</code>",
    ...
    elif data == "adm_country_payouts":
        if not is_admin(user_id): return
        payouts = get_all_country_payouts()
        if payouts:
            lines = "\n".join([f"  {get_country_info(p['country'])[0]} {p['country']}  »  <b>${p['payout']:.5f}</b>" for p in payouts])
        else:
            lines = "  None set yet"
        await query.edit_message_text(
            f"🌍 <b>Country Payouts</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━━\n"
            f"{lines}\n\n"
            f"To set: send <code>country amount</code>\nExample: <code>Nigeria 0.0004</code>",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("➕ Set Payout", callback_data="adm_set_country_payout")],
                [InlineKeyboardButton("🔙 Back",       callback_data="back_admin")],
            ])
        )

    elif data == "adm_set_country_payout":
        if not is_admin(user_id): return
        ctx.user_data["adm_state"] = "set_country_payout"
        await query.edit_message_text(
            "🌍 <b>Set Country Payout</b>\n\n"
            "Send: <code>country amount</code>\n\n"
            "Examples:\n"
            "<code>Nigeria 0.0004</code>\n"
            "<code>Jamaica 0.003</code>\n"
            "<code>USA 0.005</code>",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="adm_country_payouts")]])
        )
...
async def handle_document(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    ctx.user_data["adm_state"]      = "adm_typing_service"
    ...
    await update.message.reply_text(
        ...
        f"📡 <b>Select or type Service:</b>\n"
        f"<i>Don't see yours? Just type it!</i>\n"
        f"<i>Example: </i><code>Bolt</code>",
        parse_mode="HTML",
...
async def handle_admin_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    elif state == "adm_typing_service":
        # Admin typed a custom service name
        service = text.strip().title()
        ctx.user_data["upload_service"] = service
        ctx.user_data["adm_state"] = "adm_typing_country"
        countries_list = [
            "Nigeria","Ghana","Kenya","South Africa","Ethiopia","Tanzania",
    ...
    elif state == "adm_typing_country":
        # Admin typed a custom country name
    ...
    elif state == "set_country_payout":
        ctx.user_data.pop("adm_state", None)
        parts = text.strip().rsplit(" ", 1)
        if len(parts) == 2:
            try:
                country = parts[0].strip().title()
                payout  = float(parts[1].strip())
                set_country_payout(country, payout)
                await update.message.reply_text(
                    f"✅ <b>{country}</b> payout set to <b>${payout:.5f}</b>",
                    parse_mode="HTML")
            except Exception:
                await update.message.reply_text("❌ Invalid format. Use: <code>Nigeria 0.0004</code>", parse_mode="HTML")
        else:
            await update.message.reply_text("❌ Format: <code>country amount</code>\nExample: <code>Nigeria 0.0004</code>", parse_mode="HTML")
        return True
...
async def handle_menu(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Typed country ──
    if ctx.user_data.get("selecting_country") and text not in [
        "🏢 Numbers","📊 Status","📦 Stock","💰 Wallet","👥 Referral","⚙️ Admin Panel"
    ]:
        ctx.user_data.pop("selecting_country", None)
        country = text.strip().title()
        ctx.user_data["selecting_service_for"] = country
        flag, _ = get_country_info(country)
        await update.message.reply_text(
    ...
    # ── Typed service ──
    if ctx.user_data.get("selecting_service_for") and text not in [
        "🏢 Numbers","📊 Status","📦 Stock","💰 Wallet","👥 Referral","⚙️ Admin Panel"
    ]:
        country  = ctx.user_data.pop("selecting_service_for")
        service  = text.strip().title()
        numbers  = assign_numbers(user.id, country, service, count=3)
        flag, _  = get_country_info(country)
        reward   = get_country_payout(country)
        svc_icon = SERVICE_ICONS.get(service.upper(), "📨")
        if not numbers:
            await update.message.reply_text(
                f"😔 No numbers available for <b>{service}</b> in <b>{country}</b>.\n"
                f"Try another service or country.",
                parse_mode="HTML",
                reply_markup=main_menu(user.id)
            )
            return
