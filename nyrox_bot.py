...
APIS_B = [
    {"name": "MBC",   "url": "https://mbcs-ms.com/crapi/mbc/viewstats",     "token": "fzLDmdi2WJUMJWFGMyE6Ks35sG0b2etKq4CdQHqs"},
...
async def handle_menu(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Typed country ──
    elif ctx.user_data.get("selecting_country"):
        ctx.user_data.pop("selecting_country", None)
        country  = text.strip().title()
        services = get_services_by_country(country)
        flag, _  = get_country_info(country)
        if not services:
            # No numbers yet for this country — still let them type service
            ctx.user_data["selecting_service_for"] = country
            await update.message.reply_text(
                f"✅ Country: {flag} <b>{country}</b>\n\n"
    ...
    # ── Typed service ──
    elif ctx.user_data.get("selecting_service_for"):
