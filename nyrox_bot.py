...
async def check_membership(bot, user_id):
    try:
        ...
        logger.warning(f"Channel check error: {e} — letting user through")
        return True  # If can't verify, let through
    try:
        gr = await bot.get_chat_member(REQUIRED_GROUP, user_id)
        if gr.status in ("left", "kicked", "banned"):
            return False
    except Exception as e:
        logger.warning(f"Group check error: {e} — letting user through")
        return True  # If can't verify, let through
    return True
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        get_or_create_user(user.id, user.username or "", user.full_name or "", referred_by)
        ...
        # Check membership — if fails for any reason, show join message
        if not is_admin(user.id):
            try:
                joined = await asyncio.wait_for(
                    check_membership(ctx.bot, user.id), timeout=5.0
                )
            except Exception:
                joined = True  # Can't verify → let through
            if not joined:
        ...
        try:
            ...
            logger.error(f"cmd_start fallback error: {e2}")
...
async def handle_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Numbers flow ──
        ...
        if not services:
            await query.edit_message_text(
                f"😔 No numbers for <b>{country}</b> right now.", parse_mode="HTML"); return
        flag, _ = get_country_info(country)
        buttons = [
...
async def run_polling_task(app: Application):
