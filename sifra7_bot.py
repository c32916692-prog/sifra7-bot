...
def mark_payment_sent(wid):
    conn = get_conn()
    conn.execute("UPDATE withdrawals SET status='paid' WHERE id=?", (wid,)); conn.commit(); conn.close()
...
# tracks {wid: [msg_id_admin1, msg_id_admin2]} so both messages can be deleted
_withdrawal_msgs: dict = {}
...
def mark_number_used(number):
    # Keep assigned_to so later OTPs on this same number still know which
    # user it belongs to (needed so ALL messages forward privately, not just the first).
    conn = get_conn()
    conn.execute("UPDATE numbers SET status='used' WHERE number=?", (number,))
    conn.commit(); conn.close()
...
async def process_and_forward(bot, sms):
    ...
    # Always use the admin-configured reward. (Previously the API's own
    # "payout" field could override this, which is why amounts didn't
    # match what was set in the admin panel.)
    payout = get_otp_reward()
    ...
    if number_row and number_row["assigned_to"] and number_row["status"] in ("assigned", "used"):
        # "assigned" = first OTP on this number, "used" = a later OTP on a
        # number we already processed once. Either way it still belongs to
        # this user, so keep forwarding + crediting every time (matches the group).
...
async def handle_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Admin approve/reject/paid withdrawal ──
    elif data.startswith("adm_approve:"):
        if not is_admin(user_id):
            await query.answer("⛔ Admins only!", show_alert=True); return
        parts = data.split(":")
        target_uid = int(parts[1]); amount = float(parts[2]); wid = int(parts[3])
        ...
        # Delete message from BOTH admins
        ...
        await ctx.bot.send_message(user_id, f"✅ Approved withdrawal #{wid} for user <code>{target_uid}</code>", parse_mode="HTML")

    elif data.startswith("adm_reject:"):
        if not is_admin(user_id):
            await query.answer("⛔ Admins only!", show_alert=True); return
        parts = data.split(":")
        target_uid = int(parts[1]); wid = int(parts[2])
        w = get_withdrawal_by_id(wid)
        if not w or w["status"] != "pending":
            await query.answer("⚠️ Already processed.", show_alert=True)
            await query.delete_message(); return
        reject_withdrawal(wid)  # also restores balance
        # Delete message from BOTH admins
        for (adm, mid) in _withdrawal_msgs.pop(wid, []):
            try: await ctx.bot.delete_message(adm, mid)
            except: pass
        ...
        await ctx.bot.send_message(user_id, f"❌ Rejected withdrawal #{wid} — balance restored.", parse_mode="HTML")
        ...
        if not w or w["status"] not in ("pending","approved"):
            await query.answer("⚠️ Already marked or not found.", show_alert=True)
            await query.delete_message(); return
        ...
        try:
            await ctx.bot.send_message(w["user_id"],
                f"💸 <b>Payment Sent!</b>\n\nYour withdrawal of <b>${w['amount']:.5f}</b> has been paid.\n✅ Check your wallet/account.",
                parse_mode="HTML")
        except Exception: pass
        await ctx.bot.send_message(user_id, f"💸 Withdrawal #{wid} marked as <b>Payment Sent</b>.", parse_mode="HTML")
    ...
    # ── Admin panel ──
        ...
        await query.edit_message_text(f"💸 <b>{len(pending)} Pending Withdrawal(s)</b>\nSending each below...", parse_mode="HTML")
        # Send each withdrawal as a separate message to BOTH admins with action buttons
        for w in pending[:20]:
            wid = w["id"]
            if w["method"] == "trx":
                pay_info = f"💎 TRX:\n<code>{w['wallet']}</code>"
            else:
                bname = w.get("bank_name") or "N/A"
                acct  = w.get("account_number") or "N/A"
                pay_info = f"🏦 Bank: <b>{bname}</b>\nAccount: <code>{acct}</code>"
            msg_text = (
            ...
            for admin in ADMIN_IDS:
                try:
                    sent_msg = await ctx.bot.send_message(admin, msg_text, parse_mode="HTML", reply_markup=kb)
                    _withdrawal_msgs[wid].append((admin, sent_msg.message_id))
                except Exception as e:
                    logger.error(f"Could not send withdrawal to admin {admin}: {e}")
